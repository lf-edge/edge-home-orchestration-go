...
func TestStartMQTTClient(t *testing.T) {
	t.Run("Success", func(t *testing.T) {
		...
		err := StartMQTTClient(Host, "testClient1", port)
		expected := ""
		...
		mqttClient := CheckifClientExist(Host)
		if mqttClient != nil && mqttClient.IsConnected() {
			err := mqttClient.Publish("Test data for testing", "home/livingroom")
			if err != nil {
				t.Error(unexpectedFail, err)
			}
		}
		...
		if mqttClient != nil && mqttClient.IsConnected() {
			err := mqttClient.Subscribe("home/livingroom", "testClient")
			if err != nil {
				t.Error(unexpectedFail, err)
			}
		}
		...
		err = GetPublishedData("home/livingroom", "testClient", Host)
		if !strings.Contains(err, expected) {
			t.Error(unexpectedFail, err)
		}

		err = GetPublishedData("home/livingroom", "testClient2", Host)
		if strings.Compare(err, "Client is not subscribed to the topic home/livingroom") != 0 {
			t.Error(unexpectedFail)
		}
	...
	t.Run("SecureFail", func(t *testing.T) {
		...
		initializeTest(InvalidHost, "testClient")
		InitMQTTData()
		certificateFilePath = fakeCertsPath
		err := StartMQTTClient(InvalidHost, "testClientFailure", 8883)
		expected := ""
		if strings.Contains(err, expected) {
			t.Error(unexpectedSuccess, err)
		}
	})
}
...
func TestCheckforConnection(t *testing.T) {
	t.Run("Success", func(t *testing.T) {
		...
		err := StartMQTTClient(Host, "CheckConnection", port)
		expectedInt := ""
		if !strings.Contains(err, expectedInt) {
			t.Error(unexpectedFail, err)
		}
		...
		isConnected = checkforConnection(Host, mqttClient, port)
		if isConnected != expected {
			t.Errorf("Expected %d But received %d", expected, isConnected)
		}
		client.Disconnect(1)
